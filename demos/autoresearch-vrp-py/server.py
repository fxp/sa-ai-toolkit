"""
Standalone FastAPI server for AutoResearch-VRP demo.
"""
...
app = FastAPI(title="AutoResearch-VRP", version="1.0.0")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])


@app.get("/health")
async def health():
    return {"status": "ok", "demo": "autoresearch-vrp"}
...
async def vrp_post(action: str = "iterate", body: dict = Body(default={})):
    ...
    if action == "iterate":
        return run_iteration(int(body.get("iter_idx", 0)), lang=lang)
    if action == "full":
