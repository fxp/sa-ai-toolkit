...
def build_crew(act=None, demo_mode=False):
    ...
    all_tasks.append(("act1", Task(
        description=f"【Act1·MCP工具编排】为招商银行设计MCP跨系统方案：连接CRM客户系统+征信数据库+OA审批系统，演示客户经理一句话完成授信审批报告。{max_w}",
        expected_output="MCP编排方案",
        agent=agents["mcp_architect"],
    )))
    all_tasks.append(("act2", Task(
        description=f"【Act2·知识库】使用karpathy_knowledge_base工具，为招商银行编译信贷规则库。输入'编译知识库，提取所有核心概念'。然后回答：小微企业抵押贷款的LTV上限是多少？。{max_w}",
        expected_output="知识库编译结果",
        agent=agents["knowledge_engineer"],
    )))
    all_tasks.append(("act3", Task(
        description=f"【Act3·看板】使用ceo_dashboard工具，为招商银行的分行行长生成行长经营看板。输入数据：今日存款余额¥480亿(+3%)，贷款不良率1.2%，风险：大客户A授信到期续签。{max_w}",
        expected_output="管理看板",
        agent=agents["ceo_advisor"],
    )))
    all_tasks.append(("act3", Task(
        description=f"【Act3·监督员】使用ai_supervisor工具处理招商银行的贷款审批工单：'1.个人房贷月供查询 2.客户投诉贷款利率不合理 3.账户转账操作确认 4.授信额度调整申请 5.要求联系行长投诉'。分析自动处理率和人工审核原因。{max_w}",
        expected_output="监督员审核报告",
        agent=agents["supervisor"],
    )))
    all_tasks.append(("act3", Task(
        description=f"【Act3·智能阅读】使用hypothesis_annotator工具分析招商银行的监管政策/研报/合规通知：https://en.wikipedia.org/wiki/Artificial_intelligence。用4种标注类型（🔴关键/🟡非共识/🟢策略/💬批评）输出标注结果。{max_w}",
        expected_output="文档标注报告",
        agent=agents["pm_reader"],
    )))
    all_tasks.append(("act4", Task(
        description=f"【Act4·战略辩论】模拟招商银行的行长（支持：效率+竞争力）/合规总监（反对：监管风险）/CTO（建议先POC）三方辩论：是否引入AI贷款审批系统，替代50%人工审核。关键数字：信贷审批人力成本8000万/年。风险点：银保监会对AI决策的监管要求。输出3轮交锋+共识方案。100字以内。{max_w}",
        expected_output="战略辩论记录",
        agent=agents["strategy_debater"],
    )))
    all_tasks.append(("act4", Task(
        description=f"【Act4·一人军团】招商银行的产品经理/业务分析师规划'客户流失预警看板：NPS+留存率+趋势图+离行预测'MVP。用/office-hours格式输出5个核心问题+技术选型+功能列表。然后用playwright_frontend_test工具测试https://example.com验证能力。100字以内。{max_w}",
        expected_output="MVP规划+测试",
        agent=agents["opc_builder"],
    )))
    all_tasks.append(("act4", Task(
        description=f"【Act4·科研】使用auto_research工具，输入'AI授信审批对小微企业不良率的影响研究'。展示15-Stage管线：文献综述(关键词:AI credit scoring SME default rate NPL)→实验设计→预期指标→论文摘要。100字以内。{max_w}",
        expected_output="科研方案",
        agent=agents["researcher"],
    )))
    all_tasks.append(("act4", Task(
        description=f"【Act4·PPT压轴🎤】使用ppt_generator工具，生成招商银行的月度经营分析报告：存款¥480亿(+3%)，贷款不良率1.2%，中间业务¥8700万，零售业务增长12%。品牌规范：招行红+深蓝，右上角加密级标识。这是全流程的最终产出，展示全行28个分行，每月28份月报的价值。100字以内。{max_w}",
        expected_output="汇报PPT",
